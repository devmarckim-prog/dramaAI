...
function renderNav(){
  ...
  const isGuestMode = localStorage.getItem('ds_guest_mode') === 'true';
  ...
  if(token || isGuestMode){
    ...
    const avatarHtml = avatarUrl 
      ? `<img src="${avatarUrl}" style="width:32px; height:32px; border-radius:50%; object-fit:cover; border:1px solid var(--border)">`
    ...
    nav.innerHTML=`
      <div class="nav-user-info" style="display:flex; align-items:center; gap:12px">
        ...
        <div class="nav-avatar" onclick="showPage('settings')" title="설정" style="width:32px; height:32px; cursor:pointer; display:flex; align-items:center; justify-content:center;">
          ${avatarHtml}
        </div>
      ...
      html+=`<div class="project-card" onclick="openProject(${p.id})">
        ...
        <div class="project-card-body">
          ...
          <div class="project-card-footer">
            ...
            <button class="project-card-del" onclick="event.stopPropagation(); confirmDeleteProject('${p.id}')">&times;</button>
          </div>
        </div>
      </div>`;
    }
...
// ── 1-A단계: 핵심 기획(Core) 프롬프트 ──
const PROMPT_CORE = DRAMA_BASE_ROLE + `
  ...
  "episodes": [
    {
      ...
      "scenes": [
        ...
        {"num": "S#3", "loc": "장소 / 시간", "chars": ["인물1", "인물2"]},
        {"num": "S#4", "loc": "장소 / 시간", "chars": ["인물1"]},
        {"num": "S#5", "loc": "장소 / 시간", "chars": ["인물1", "인물2"]},
        {"num": "S#6", "loc": "장소 / 시간", "chars": ["인물1"]},
        {"num": "S#7", "loc": "장소 / 시간", "chars": ["인물1", "인물2"]},
        {"num": "S#8", "loc": "장소 / 시간", "chars": ["인물1", "인물2"]}
      ]
    }
  ]
}
...
// ── 1-B단계: 상세 기획(Detailed Plan) 프롬프트 ──
const PROMPT_PLAN_DETAIL = DRAMA_BASE_ROLE + `

======================
 1-B단계: 상세 기획 및 전체 회차 구성
======================
이미 생성된 핵심 컨셉을 바탕으로 나머지 회차(2~8화)의 상세 씬과 전체 갈등 구조, 유사 레퍼런스를 생성합니다.
아래 JSON 스키마만 반환하세요.
  ...
  "episodes": [
    {
      "num": 1,
      "scenes": [
        {"num":"S#1", "desc":"씬 상세 묘사 2~3문장"},
        {"num":"S#2", "desc":"씬 상세 묘사 2~3문장"},
        {"num":"S#3", "desc":"씬 상세 묘사 2~3문장"},
        {"num":"S#4", "desc":"씬 상세 묘사 2~3문장"},
        {"num":"S#5", "desc":"씬 상세 묘사 2~3문장"},
        {"num":"S#6", "desc":"씬 상세 묘사 2~3문장"},
        {"num":"S#7", "desc":"씬 상세 묘사 2~3문장"},
        {"num":"S#8", "desc":"씬 상세 묘사 2~3문장"}
      ]
    },
    { "num": 2, "title": "...", "logline": "...", "story": "...", "scenes": [...] },
    { "num": 3, "title": "...", "logline": "...", "story": "...", "scenes": [...] }
    // ... 나머지 모든 회차 반복
  ],
  "similar": {
...
주의: 이미 결정된 1화의 씬 번호와 장소를 유지하면서 상세 묘사(desc)를 추가하고, 나머지 모든 회차를 완성하세요.
순수 JSON만 반환하세요.`;
...
async function callAPI_Core(input){
  const prompt=`${_buildBaseContext(input)}

위 설정으로 드라마의 핵심 컨셉(제목, 로그라인, 줄거리, 주인공 성격)과 1화의 씬 목록(장소/인물만)을 생성해주세요.
순수 JSON만 반환.`;
  return await _callAPI('plan_core', PROMPT_CORE, prompt, 4000);
}

async function callAPI_Plan_Detail(input, coreData){
  const prompt=`${_buildBaseContext(input)}
...
1화의 씬 목록(장소/인물)은 이미 결정되었으니 여기에 상세 묘사(desc)만 추가하고, 2화부터는 새롭게 구성하세요.
순수 JSON만 반환.`;
  return await _callAPI('plan_detail', PROMPT_PLAN_DETAIL, prompt, 15000);
}
...
async function callAPI_Script(input, planData, epIdx, startIdx=0, endIdx=null){
  const ep=planData?.episodes?.[epIdx];
  if(!ep) throw new Error('에피소드 데이터 없음');
  const totalScenes = ep.scenes.length;
  endIdx = endIdx || totalScenes;
  
  const chunkScenes = ep.scenes.slice(startIdx, endIdx);
  const charLines=(input.chars||[]).map(c=>`${c.name||'?'}(${c.role||'?'}, ${c.job||'?'}, ${c.personality||'?'})`).join(' / ');
  
  let prevCtx='';
  if(epIdx>0 && startIdx===0){
    const prev=planData.episodes[epIdx-1];
    prevCtx=`\n이전화(${epIdx}화) 엔딩:"${prev?.ending||''}"\n위 엔딩 감정에서 자연스럽게 이어질 것.`;
  } else if (startIdx > 0 && window._scripts[epIdx]) {
    const recent = window._scripts[epIdx].slice(-2).map(sc => {
      let txt = `${sc.heading}\n`;
      txt += (sc.lines||[]).map(l => l.type==='action' ? l.text : (l.type==='dialog' ? `${l.char}: ${l.line}` : '')).join('\n');
      return txt;
    }).join('\n\n');
    prevCtx=`\n━━━ 직전 작성 내용 (바로 이어서 작성) ━━━\n${recent}`;
  }
  
  const scenesDetail = chunkScenes.map(s=>`${s.num||''} | ${s.loc||''} | 등장:${(s.chars||[]).join('·')} | ${s.desc||''}`).join('\n');
...
async function generateScriptForEp(epIdx){
  ...
  const total = planData.episodes[epIdx].scenes.length;
  const chunkSize = 4;
  window._scripts[epIdx] = [];
  
  try{
    // 선행 청크
    addDebugLog(`[대본] ${epIdx+1}화 1~4씬 선행 생성 시작...`, 'info');
    const result = await callAPI_Script(currentInput, planData, epIdx, 0, chunkSize);
    if(result?.script?.length){
      ...
      renderSceneRow();
      renderSceneDetail(0);
      showToast(`${epIdx+1}화 1~4씬 선공개! 뒷부분 생성 중...`,'success','?',3000);
      if(btn){ btn.textContent='백그라운드 생성중..'; }
      updateScriptSidebar();
    }
  } catch(e){
    showToast(`대본 1차 생성 오류: ${e.message}`,'warn','??',5000);
  ...
  if(total > chunkSize) {
    (async () => {
      for(let i=chunkSize; i<total; i+=chunkSize){
        addDebugLog(`[대본] ${epIdx+1}화 ${i+1}~${Math.min(i+chunkSize, total)}씬 생성...`, 'info');
        try {
          const chunkN = await callAPI_Script(currentInput, planData, epIdx, i, i+chunkSize);
          if(chunkN?.script?.length){
            window._scripts[epIdx] = window._scripts[epIdx].concat(chunkN.script);
            if(currentEpIdx === epIdx) {
              aiScript = window._scripts[epIdx];
              const oldActive = currentSceneIdx;
              renderSceneRow();
              renderSceneDetail(oldActive);
            }
            showToast(`${epIdx+1}화 ${i+1}~${Math.min(i+chunkSize, total)}씬 추가 완료!`, 'info', '?', 3000);
          }
        } catch(e) {
          addDebugLog(`[대본] ${epIdx+1}화 잔여 씬 생성 오류: ${e.message}`, 'error');
        }
      }
...
function applyApiBudget(breakdown, epCount, stats){
  // 요약 카드
  if(stats){
    const tots = breakdown.map(ep=>Object.values(ep.items||{}).reduce((s,v)=>s+parseInt(v||0),0));
    ...
    if(btabs) btabs.innerHTML=tl.map((l,i)=>`<button class="btab ${i===0?'active':''}" onclick="switchApiBTab(${i})">${l}</button>`).join('');
    switchApiBTab(0);
...
async function startGenerationFlow(){
  ...
  // 2. 단계별 생성 시작 (Background Flow)
  (async function mainBackgroundFlow(){
    try {
      // PHASE 1: CORE 
      updateApiStatus('핵심 컨셉 설계 중...', '기획안(Core) 생성');
      const coreData = await callAPI_Core(currentInput);
      window._planData = coreData;
      applyPlanResult(coreData, currentInput); // UI 반영
      ...
      currentEpIdx = 0;
      buildResultPanels(); 
      updateApiStatus('핵심 컨셉 로드 완료, 상세 내용 구성 중...', '상세 기획 생성 중');

      // PHASE 2 & 3 & 4 (병렬 및 백그라운드)
      const detailPlanPromise = callAPI_Plan_Detail(currentInput, coreData);
      const prodPromise = callAPI_Production(currentInput);
      
      const detailedData = await detailPlanPromise;
      window._planData = { ...window._planData, ...detailedData };
      applyPlanResult(window._planData, currentInput); // 상세 정보로 UI 최종 반영
      aiEpisodes = window._planData.episodes; 
      buildResultPanels(); 
      await updateCard(50, 0);

      const pplPromise = callAPI_PPL(currentInput, window._planData);
      const [prodData, pplData] = await Promise.all([prodPromise, pplPromise]);
      ...
      buildResultPanels();
      updateApiStatus('제작 기획 완료, 1화 대본 집필 중...', '대본 생성');

      // PHASE 5: SCRIPT
      await generateScriptForEp(genId, 0); 
      
      await finishGenerate(false);
      updateApiStatus('모든 생성 작업이 완료되었습니다.', '완료');

    } catch(e) {
      console.error("Background flow error:", e);
      if(typeof addDebugLog === 'function') addDebugLog("생성 과정 오류: " + e.message, "error");
      await finishGenerate(true, e.message);
    }
  })();
}
...
async function initApp(){
  ...
  if (hash && hash.includes('access_token=')) {
    ...
    if (token) {
      ...
      // Fetch profile to get real email and avatar
      try {
        const profile = await fetchUserProfile();
        if(profile){
          localStorage.setItem('ds_user_email', profile.email);
          if(profile.user_metadata && (profile.user_metadata.avatar_url || profile.user_metadata.picture)){
            localStorage.setItem('ds_user_avatar', profile.user_metadata.avatar_url || profile.user_metadata.picture);
          }
        }
  ...
  // If already logged in but no avatar, try fetching it once
  const token = localStorage.getItem('ds_auth_token');
  if(token && !localStorage.getItem('ds_user_avatar')){
    try {
      const profile = await fetchUserProfile();
      if(profile && profile.user_metadata){
        const avatar = profile.user_metadata.avatar_url || profile.user_metadata.picture;
        if(avatar) localStorage.setItem('ds_user_avatar', avatar);
      }
    } catch(e){}
  }
...
/** 
 * [CORE] AI Generation Entry Point
 * Performs connectivity test before starting the 4-stage generation flow.
 */
async function startGenerate() {
  ...
  // 1. Move to generating page immediately for feedback
  ...
  try {
    if (typeof addDebugLog === 'function') {
      addDebugLog("--- 서버 연결 확인 후 바로 생성 시작 ---", "info");
    }

    if (typeof addDebugLog === 'function') {
      addDebugLog("서버 확인 완료. 4단계 대본 생성을 바로 시작합니다.", "success");
    }
    await startGenerationFlow();

  } catch (error) {
    console.error("생성 시작 오류:", error);
    if (typeof addDebugLog === 'function') {
      addDebugLog("생성 시작 중 오류: " + error.message, "error");
    }
    // Fallback: still try to run generation
    try { await startGenerationFlow(); } catch(e) {
      if (typeof addDebugLog === 'function') addDebugLog("생성 흐름 오류: " + e.message, "error");
    }
  }
}
/**
...
window.confirmDeleteProject = function(projectId) {
  showConfirmModal(
    '프로젝트 삭제',
    '이 프로젝트를 삭제하시겠습니까? 삭제된 프로젝트는 복구할 수 없습니다.',
    async () => {
      try {
...
window.showConfirmModal = function(title, message, onConfirm) {
  const wrap = document.getElementById('login-modal-wrap'); // 기존 모달 랩 재사용
  if (!wrap) return;

  wrap.innerHTML = `
    <div class="login-backdrop" id="modal-backdrop" onclick="closeConfirmModal(event)">
      <div class="login-box" style="padding:0; overflow:hidden; border:1px solid var(--border); box-shadow:0 10px 30px rgba(0,0,0,0.5)">
        <div class="login-hero" style="min-height:0; padding:20px 24px; background:linear-gradient(135deg, var(--paper3), var(--paper1))">
          <div class="login-logo" style="font-size:18px"><span style="color:var(--red); font-weight:700">ALERT:</span> ${title}</div>
        </div>
        <div class="login-body" style="padding:24px">
          <div class="login-sub" style="font-size:14px; color:var(--ink2); text-align:left; margin-bottom:24px; line-height:1.6">
            ${message}
          </div>
          <div style="display:flex; gap:12px; justify-content:flex-end">
            <button class="btn btn-ghost" onclick="closeConfirmModal()">취소</button>
            <button class="btn btn-primary" id="modal-confirm-btn" style="background:var(--red); color:#fff; border-color:var(--red)">삭제하기</button>
          </div>
        </div>
      </div>
    </div>`;
...
window.closeConfirmModal = function(e) {
  ...
  if (bd) {
    ...
    setTimeout(() => {
      document.getElementById('login-modal-wrap').innerHTML = '';
    }, 200);
  }
};
/**
...
window.showConfirmModal = function(title, message, onConfirm) {
  ...
  wrap.innerHTML = `
    <div class="login-backdrop" id="modal-backdrop" onclick="closeConfirmModal(event)">
      <div class="login-box" style="padding:0; overflow:hidden; border:1px solid var(--border); box-shadow:0 10px 30px rgba(0,0,0,0.5); width:400px; max-width:90%">
        <div class="login-hero" style="min-height:0; padding:20px 24px; background:linear-gradient(135deg, var(--paper3), var(--paper1))">
          <div class="login-logo" style="font-size:18px"><span style="color:var(--red); font-weight:700">ALERT:</span> ${title}</div>
        </div>
        <div class="login-body" style="padding:24px">
